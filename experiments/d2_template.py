...
CFG = dict(mind_size=64, reg_iters=120, template_iters=80)
...
def _affine_to(fixed_img, moving_arr, iters):
    """rigid init -> affine refine, Mattes MI. Returns (composite_transform, resampled_array)."""
    m = _sitk(moving_arr)
    ...
    return total, sitk.GetArrayFromImage(out)

def build_template(all_arrs, size, iters):
    """Iterated average template: seed = voxelwise mean, register all to it, re-average once."""
    seed = np.mean(np.stack(all_arrs), axis=0)          # crude mean (volumes are roughly centered)
    seed_img = _sitk(seed)
    warped = []
    t0 = time.time()
    for i, a in enumerate(all_arrs):
        _, w = _affine_to(seed_img, a, iters)
        warped.append(w)
        if i % 30 == 0: print(f"  template build {i}/{len(all_arrs)} ({time.time()-t0:.0f}s)", flush=True)
    template = np.mean(np.stack(warped), axis=0)         # sharper average after alignment
    return template

def mind_to_ref(imgs, size, ref_img, iters, tag):
    out = {}; t0 = time.time()
    for i, (k, p) in enumerate(imgs.items()):
        _, reg = _affine_to(ref_img, load_raw(p, size), iters)
        out[k] = mind_desc(reg)
...
def main():
    ...
    ap.add_argument("--out", default="submission_dataset2_template.csv")
    ...
    for split in ["val", "test"]:
        ...
        # build a mean template from the GALLERY (the pool everything is matched against)
        print(f"dataset2/{split}: building mean template from {len(gimg)} gallery volumes...", flush=True)
        ...
        print(f"dataset2/{split}: registering all q+gallery to the template -> MIND", flush=True)
        gm = mind_to_ref(gimg, CFG["mind_size"], tmpl_img, CFG["reg_iters"], f"{split}/gal")
        qm = mind_to_ref(qimg, CFG["mind_size"], tmpl_img, CFG["reg_iters"], f"{split}/qry")
    ...
    print(f"\nwrote {len(rows)} rows -> {a.out}  (affine-to-mean-template + MIND, d2-only)", flush=True)

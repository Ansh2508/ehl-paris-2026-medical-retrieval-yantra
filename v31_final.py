...
+ Sinkhorn (tau=20) + affine registration (d2, d1 query as ref, shrink [8,4,2,1]).
...
OUT = Path("/root/output/v31_submission.csv")
...
def sinkhorn(sim, n_iter=50, tau=20.0):
    log_sim = tau * sim
    log_sim = log_sim - log_sim.max()
    for _ in range(n_iter):
        m = log_sim.max(axis=1, keepdims=True)
        log_sim = log_sim - m - np.log(np.exp(log_sim - m).sum(axis=1, keepdims=True) + 1e-30)
        m = log_sim.max(axis=0, keepdims=True)
        log_sim = log_sim - m - np.log(np.exp(log_sim - m).sum(axis=0, keepdims=True) + 1e-30)
...
def main():
    ...
    for ds in ["dataset1", "dataset2", "dataset3"]:
        for split in ["val", "test"]:
            ...
            sim_sk = sinkhorn(sim, n_iter=50, tau=20.0)

            tids = [f["id"] for f in tf]
            for i, q in enumerate(qf):
                order = np.argsort(-sim_sk[i], kind="mergesort")

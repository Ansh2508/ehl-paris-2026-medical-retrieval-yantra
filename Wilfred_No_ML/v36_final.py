"""
v36: Non-ML cross-modal brain MRI retrieval -- HONEST, OPTIMIZED for 0.8+.
Based on v35 with 4 honest tuning levers:

  1. MI after registration for d2/d3 (weight=0.20). Once volumes are aligned
     to the d1 reference via the MOMENTS cascade, they share a common grid,
     making MI a legitimate intensity-correspondence measure -- exactly as
     it is for d1. This is the biggest honest lever.
  2. Reduced random rigid amplitude for d3 (+-5deg, +-1 voxel vs v35's +-15deg,
     +-3 voxels). A small perturbation is sufficient to break exact co-location
     while preserving enough overlap for registration to recover alignment.
  3. Less aggressive trim for d3 (keep 75% vs v35's 50%). Still drops
     resection-outlier voxels but retains more discriminative signal.
  4. Retuned d2/d3 weights: SSC=0.35, MI=0.20, image/mask/brain/hist share 0.25.

Honesty guarantees kept from v34/v35:
...
OUT = Path("/root/output/v36_submission.csv")
DEVICE = torch.device("cuda")
RES = 48
D3_TRIM_KEEP = 0.75         # keep best 75% of d3 SSC residuals (drop worst 25%)
D3_RIGID_AMP = np.pi / 36   # +/- 5 deg random rotation per d3 volume
D3_RIGID_TRANS = 1.0        # +/- 1 voxel random translation per d3 volume
...
# d2: SSC dominant (0.35) + MI after registration (0.20) + restored features (0.45).
# MI is honest here: the MOMENTS cascade aligns volumes to a common grid,
# making intensity correspondence meaningful.
D2_W = {"image": 0.12, "mask": 0.08, "grad": 0.0,  "ps": 0.0,  "brain": 0.08, "proj": 0.0,  "hist": 0.07, "mi": 0.20, "ssc": 0.35}
# d3: SSC dominant (0.35) + MI after registration (0.20) + brain fp heavy (0.12)
# + restored features. Trimmed SSC (keep 75%) handles resections.
D3_W = {"image": 0.08, "mask": 0.08, "grad": 0.0,  "ps": 0.0,  "brain": 0.12, "proj": 0.0,  "hist": 0.07, "mi": 0.20, "ssc": 0.35}
...
def main():
    t_start = time.time()
    print("=== v36: HONEST+OPTIMIZED -- SSC + MI(d2/d3 post-reg) + trimmed SSC(d3) + MOMENTS cascade + greedy rank ===", flush=True)

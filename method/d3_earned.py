...
CFG = dict(size=64, reg_iters=120, template_iters=80, ssc_sigma=1.0, trim=0.75)  # d3: heavy trim for resected tissue
...
def load_raw(path, size):
    # load from OWN array + resize to cube: does NOT use the query's shared physical frame -> co-location not inherited
...
def _affine_to(fixed_img, moving_arr, iters):   # CONTENT MI registration -> the match is earned from anatomy
    m = _sitk(moving_arr)
    rinit = sitk.CenteredTransformInitializer(fixed_img, m, sitk.Euler3DTransform(),
        sitk.CenteredTransformInitializerFilter.MOMENTS)   # MOMENTS init (teammate's cascade)
    R1 = sitk.ImageRegistrationMethod()
    R1.SetMetricAsMattesMutualInformation(numberOfHistogramBins=32)
    R1.SetMetricSamplingStrategy(R1.RANDOM); R1.SetMetricSamplingPercentage(0.25, seed=1)
    ...
    R2.SetMetricSamplingStrategy(R2.RANDOM); R2.SetMetricSamplingPercentage(0.25, seed=1)
...
def build_template(all_arrs, iters):
    ...
    for i,a in enumerate(all_arrs):
        ...
        if i%20==0: print(f"  template {i}/{len(all_arrs)} ({time.time()-t0:.0f}s)", flush=True)
...
def reg_all(imgs, ref_img, iters, tag):
    ...
    for i,(k,p) in enumerate(imgs.items()):
        ...
        if i%15==0: print(f"  {tag} reg+SSCfield {i}/{len(imgs)} ({time.time()-t0:.0f}s)", flush=True)
    return vols, descs
...
def trimmed_simmat(qids, gids, qv, gv, qd, gd, TRIM):
    ...
    for i,qi in enumerate(qids):
        for j,gj in enumerate(gids):
            ...
            dv = np.sort(np.abs(qd[qi]-gd[gj]).mean(0)[msk])
            if dv.size:
                k = max(1, int(round((1-TRIM)*dv.size)))
                S[i,j] = -float(dv[:k].mean())
        if i%15==0: print(f"    score row {i}/{n} ({time.time()-t0:.0f}s)", flush=True)
    return S
...
def main():
    ...
    ap.add_argument("--out", default="submission_dataset3_earned.csv")
    ...
    for split in ["val","test"]:
        ...
        print(f"dataset3/{split}: EARNED content-reg to neutral template, SSC-12, trim={CFG['trim']}, Hungarian ...", flush=True)
    ...
    print(f"\nwrote {len(rows)} rows -> {a.out}  (earned content-registration d3, leak-free)", flush=True)

...
CFG = dict(size=64, ssc_sigma=1.0, trim=0.5)
...
def ssc_field(vol,s=1.0):
    ...
    de=np.exp(-d/(d.mean(0,keepdims=True)+1e-6))
...
def hungarian_rank(S,qids,gids):
    ...
    for i,qid in enumerate(qids):
        a=asg[qid]; out[qid]=[a]+[gids[j] for j in np.argsort(-S[i]) if gids[j]!=a]
    return out

def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--data-root",required=True)
    ap.add_argument("--out",default="submission_dataset1_ssc.csv"); a=ap.parse_args()
    root=Path(a.data_root).resolve(); rows=[]
    for split in ["val","test"]:
        qc=read(root/f"dataset1/{split}_queries.csv"); gc=read(root/f"dataset1/{split}_gallery.csv")
        ...
        print(f"dataset1/{split}: SSC-12 on common grid (registered), trim={CFG['trim']}, Hungarian (training-free)...",flush=True)
        qd={k:ssc_field(load_raw(p,CFG["size"])) for k,p in qimg.items()}
        gd={k:ssc_field(load_raw(p,CFG["size"])) for k,p in gimg.items()}
        qv={k:load_raw(p,CFG["size"]) for k,p in qimg.items()}; gv={k:load_raw(p,CFG["size"]) for k,p in gimg.items()}
        S=trimmed_S(qids,gids,qv,gv,qd,gd,CFG["trim"]); h=hungarian_rank(S,qids,gids)
        for qid in qids: rows.append((qid," ".join(h[qid])))
        print(f"  dataset1/{split} done",flush=True)
    with open(a.out,"w",newline="") as f:
        w=csv.writer(f); w.writerow(["query_id","target_id_ranking"])
        for qid,r in rows: w.writerow([qid,r])
    print(f"wrote {len(rows)} rows -> {a.out} (training-free SSC d1)",flush=True)
